...
def index(request):
    ...
    young_donors_appointments = DonationAppointment.objects.filter(
        ...
        age=current_year - F('donor__dob__year')
    ).filter(age__gte=18, age__lte=25).count()

    context = {
        'faqs': FAQ.objects.all(),
        'testimonials': Testimonial.objects.all(),
        'donors_registered_this_year': donors_registered_this_year,
        'appointments_this_year': appointments_this_year,
        'young_donors_appointments': young_donors_appointments,
        'current_year': current_year,
    }
    return render(request, 'index.html', context)
...
def submit_testimonial(request):
    ...
    try:
        ...
        return render(request, 'error.html', {'message': 'You are not registered as a donor.'})

    if request.method == 'POST':
        form = TestimonialForm(request.POST)
        if form.is_valid():
            testimonial = form.save(commit=False)
            testimonial.donor = donor
            testimonial.save()
            return redirect('testimonial_thanks')  # Redirect after submission
    else:
...
def donor(request):
    ...
    if request.method == 'POST' and 'submit_testimonial' in request.POST:
        ...
        if testimonial_form.is_valid():
            ...
            testimonial.save()
            success = True
            testimonial_form = TestimonialForm()  # Reset form after success
...
def donor_forget_reset(request):
    if request.method == 'POST':
        ...
        try:
            ...
            send_mail(
                subject='LifeShare Password Reset Link',
                message=f'Hi {donor.first_name},\n\nClick the link below to reset your password:\n{reset_link}\n\nIf you did not request this, ignore this email.',
                from_email=settings.DEFAULT_FROM_EMAIL,
                recipient_list=[donor.email],
                fail_silently=False,  # ✅ Leave this as False so errors show
            )

            return render(request, 'password_reset_email_sent.html', {'email': email})

        except Donor.DoesNotExist:
            return render(request, 'donor_forget_reset.html', {'error': 'No donor found with that email.'})

    return render(request, 'donor_forget_reset.html')


def reset_donor_password(request, token):
    try:
        donor = Donor.objects.get(reset_token=token)
    except Donor.DoesNotExist:
    ...
    if request.method == 'POST':
        ...
        if password == confirm:
            donor.password = make_password(password)
            donor.reset_token = None
            donor.save()
            return render(request, 'reset_success.html')
        else:
            return render(request, 'reset_form.html', {'token': token, 'error': 'Passwords do not match'})
...
def generate_reports(request):
    ...
    filters = Q(collector=collector)
    if selected_month:
        try:
            year, month = map(int, selected_month.split('-'))
            filters &= Q(appointment_date__year=year, appointment_date__month=month)
        except:
            pass

    if selected_blood_type:
        filters &= Q(donor__blood_type=selected_blood_type)

    appointments = DonationAppointment.objects.filter(filters)

    total_donations = appointments.count()
    donor_ids = appointments.values_list('donor_id', flat=True).distinct()
    donor_list = Donor.objects.filter(id__in=donor_ids).annotate(
        last_appointment=Max('appointments__appointment_date')
    )

    donation_counts = appointments.values('donor__blood_type').annotate(count=Count('id')).order_by('-count')
...
def export_report_csv(request):
    ...
    donation_data = DonationAppointment.objects.filter(filters).values('donor__blood_type').annotate(count=Count('id')).order_by('-count')

    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename="donation_report.csv"'

    writer = csv.writer(response)
    writer.writerow(['Blood Type', 'Donations'])

    for item in donation_data:
        writer.writerow([item['donor__blood_type'], item['count']])
...
def export_report_pdf(request):
    ...
    context = {
        ...
        'blood_type': selected_blood_type,
    }
